...
def craurl(keyword, flag, current_url):
    try:
        base_url = 'https://www.google.com/search?q=' #접속할 url
        if flag == 0:
            url = base_url + quote_plus(keyword)
        else:
            url = current_url
        # driver = webdriver.Chrome(path, chrome_options=options)
        driver = webdriver.Chrome("/Users/jungwoo/Desktop/dev/SpringBootApiServer/searchModule/config/chromedriver", chrome_options=options)
    ...
    for j in range(2):
        ...
        try:
            ...
            # driver.find_element(By.ID, '//*[@id="pnnext"]').click()#다음페이지로이동
    ...
    try:
        ...
        for image_url in soup.find_all("div", style=re.compile("background-image")):
            ...
            if len(extract_url) > 131072:
                continue
            column_count = extract_url.count("\"")
            double_column_count = extract_url.count('\'')
            if column_count >= 2 or double_column_count >= 2:
                if len(result) <= 15:
...
def banner_crawling_function(url):
    banner_url_list = crawel(url)
    if len(banner_url_list) == 0:
        return False
    else:
        cm.write_banner_url_list_csv(banner_url_list)
        return True
